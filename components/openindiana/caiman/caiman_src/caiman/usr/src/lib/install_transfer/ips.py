...
class InstallCLIProgressTracker(progress.NullProgressTracker):
    ...
    def dl_output(self):
        ...
        if self._dl_cur_pkg != self.dl_cur_pkg:
            self._logger_output("Download: %s ..." % self.dl_cur_pkg)
            self._dl_cur_pkg = self.dl_cur_pkg
...
class InstallFancyProgressTracker(progress.FancyUNIXProgressTracker):
    ...
    def dl_output(self, force=False):
        ...
        if self._dl_cur_pkg != self.dl_cur_pkg:
            self.trans_logger.debug("Download: %s ..." % self.dl_cur_pkg)
            self._dl_cur_pkg = self.dl_cur_pkg
